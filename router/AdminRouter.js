    ...
    try {
        ...
        if (req.files.length > 0) {
            ...
            for (let i = 0; i < req.files.length; i++) {
                ...
                let link =
                    "/public/" +
                    req.files[i].path.slice(index, req.files[i].path.length);
...
router.delete("/deleteProduct/:id", async (req, res) => {
    try {
        const order = await orderssModel.find().populate("product.productId");
        ...
        if (order.length > 0) {
            ...
            if (test == 0) {
                ...
                res.json({
                    mess: "sản phẩm đang có trong 1 order",
                    status: 400,
                });
            }
        } else {
    ...
    try {
        if (req.cookies.user) {
            ...
            if (req.file != undefined) {
                ...
                let link =
                    "/public/" +
                    req.file.path.slice(index, req.file.path.length);
...
router.get(
    ...
    check.checkLogin,
    check.checkAdmin,
    controllerAdmin.adminlistOrder
);
...
router.get(
    ...
    check.checkLogin,
    check.checkAdmin,
    controllerAdmin.adminlistProduct
);
...
router.get(
    ...
    check.checkLogin,
    check.checkAdmin,
    controllerAdmin.adminaddProduct
);
...
router.get(
    ...
    check.checkLogin,
    check.checkAdmin,
    controllerAdmin.adminchangePass
);
...
router.get(
    ...
    check.checkLogin,
    check.checkAdmin,
    controllerAdmin.adminchangeProfile
);
...
router.get(
    ...
    check.checkLogin,
    check.checkAdmin,
    controllerAdmin.orderUserDetail
);
